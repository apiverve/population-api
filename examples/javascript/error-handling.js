/**
 * Population Data API - Error Handling Example
 *
 * This example demonstrates proper error handling when using the Population Data API.
 ...
 */
async function callPopulationDataAPIWithErrorHandling(queryParams = {}) {
  try {
...
async function callWithRetry(maxRetries = 3, initialDelay = 1000) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const result = await callPopulationDataAPIWithErrorHandling({
